...
final class FilterSettings {
	...
	public function register_settings(): void {
		...
		add_settings_section(
			...
			__( 'Filter Visibility', 'advanced-product-filter' ),
			array( $this, 'render_section_intro' ),
			self::PAGE_SLUG
		);

		$this->register_checkbox_field( 'show_categories', __( 'Show Categories', 'advanced-product-filter' ) );
		$this->register_checkbox_field( 'show_brands', __( 'Show Brands', 'advanced-product-filter' ) );
		$this->register_checkbox_field( 'show_price', __( 'Show Price', 'advanced-product-filter' ) );
		$this->register_checkbox_field( 'show_rating', __( 'Show Customer Rating', 'advanced-product-filter' ) );
		$this->register_checkbox_field( 'show_availability', __( 'Show Availability', 'advanced-product-filter' ) );
		$this->register_checkbox_field( 'show_colors', __( 'Show Color', 'advanced-product-filter' ) );
	}
	...
	public function register_menu(): void {
		add_submenu_page(
			...
			__( 'Advanced Product Filter Settings', 'advanced-product-filter' ),
			__( 'Settings', 'advanced-product-filter' ),
			'manage_woocommerce',
			self::PAGE_SLUG,
	...
	public function render_page(): void {
		...
		<div class="wf-admin-wrap">
			<div class="wf-admin-header">
				...
				<div>
					<h1><?php esc_html_e( 'Advanced Product Filter Settings', 'advanced-product-filter' ); ?></h1>
					<p><?php esc_html_e( 'Configure which filters appear in your shop sidebar', 'advanced-product-filter' ); ?></p>
				</div>
			</div>
				...
				<div class="wf-admin-card wf-admin-success-card">
					<div class="wf-admin-card-body wf-admin-card-body-compact">
						<p class="wf-admin-success-text">
							...
							<?php esc_html_e( 'Filter settings saved.', 'advanced-product-filter' ); ?>
						</p>
					</div>
				</div>
			<?php endif; ?>
			...
			<form action="options.php" method="post">
				...
				<div class="wf-admin-card">
					<div class="wf-admin-card-header">
						<h2><?php esc_html_e( 'Filter Visibility', 'advanced-product-filter' ); ?></h2>
						<p><?php esc_html_e( 'Enable or disable individual filter blocks in the shop sidebar.', 'advanced-product-filter' ); ?></p>
					</div>
						...
						<div class="wf-admin-toggle-group wf-admin-toggle-grid">
							...
							$toggles = array(
								'show_categories'   => array(
									'label'   => __( 'Show Categories', 'advanced-product-filter' ),
									'feature' => 'category_filter',
								),
								'show_brands'       => array(
									'label'   => __( 'Show Brands', 'advanced-product-filter' ),
									'feature' => 'brand_filter',
								),
								'show_price'        => array(
									'label'   => __( 'Show Price Range', 'advanced-product-filter' ),
									'feature' => 'price_filter',
								),
								'show_rating'       => array(
									'label'   => __( 'Show Customer Rating', 'advanced-product-filter' ),
									'feature' => 'rating_filter',
								),
								'show_availability' => array(
									'label'   => __( 'Show Availability', 'advanced-product-filter' ),
									'feature' => 'availability_filter',
								),
								'show_colors'       => array(
									'label'   => __( 'Show Color Filter', 'advanced-product-filter' ),
				...
				<div class="wf-admin-card">
					<div class="wf-admin-card-header">
						<h2><?php esc_html_e( 'Data Management', 'advanced-product-filter' ); ?></h2>
						<p><?php esc_html_e( 'Control what happens to Advanced Product Filter data when the plugin is removed.', 'advanced-product-filter' ); ?></p>
					</div>
					<div class="wf-admin-card-body">
						<label class="wf-admin-toggle">
							...
							<span><?php esc_html_e( 'Delete plugin data on uninstall', 'advanced-product-filter' ); ?></span>
						</label>
						<p class="wf-admin-help"><?php esc_html_e( 'If enabled, plugin options and analytics data will be removed when Advanced Product Filter is uninstalled.', 'advanced-product-filter' ); ?></p>
					</div>
				</div>
				...
				<div class="wf-admin-submit-wrap">
					<button type="submit" class="wf-admin-submit">
						...
						<?php esc_html_e( 'Save Settings', 'advanced-product-filter' ); ?>
					</button>
				</div>
			</form>
		</div>
	...
	public function render_section_intro(): void {
		echo '<p>' . esc_html__( 'Enable or disable individual filter blocks in the shop sidebar.', 'advanced-product-filter' ) . '</p>';
	}
	...
	public function render_checkbox_field( array $args ): void {
		...
		echo ' ' . esc_html__( 'Enabled', 'advanced-product-filter' );
		echo '</label>';
	}
